 ...
 * @param {boolean} [options.superMode]
 ...
 * @param {'Cheap' | 'Super'} [options.mode]
...
export async function createAiScraperApi({
  ...
  if (mode !== undefined && !["Cheap", "Super"].includes(mode)) {
...
export async function rerunAiScraperApi({
  ...
  const payload = { scraperId, url };
  if (maxDepth !== undefined) payload.maxDepth = maxDepth;
  if (maxPages !== undefined) payload.maxPages = maxPages;
  if (limit !== undefined) payload.limit = limit;
  if (includePatterns !== undefined) payload.includePatterns = includePatterns;
  if (excludePatterns !== undefined) payload.excludePatterns = excludePatterns;
  if (proxyCountry !== undefined) payload.proxyCountry = proxyCountry;
  if (maxRetry !== undefined) payload.maxRetry = maxRetry;
 ...
 * @param {{ includeHtml?: boolean }} [options]
 */
export async function getResultByIdApi(token, resultId, options = {}) {
  const headers = {
    accept: "application/json",
    ...getAuthHeaders(token),
  };
  return request("GET", `${API_BASE_URL}/results/${resultId}`, {
    headers,
