...
export async function runCli(argv = process.argv) {
  ...
  program
    ...
    .option("--skip-mcp", "Do not connect the hosted MrScraper MCP server.")
    .option("--local-mcp", "Run the MrScraper MCP package locally instead.")
    .option("--mcp-url <url>", "Connect to a self-hosted MCP URL instead.")
    ...
    .action(async (options) => {
      ...
      if (options.localMcp && options.mcpUrl) {
        throw new Error("Use either --local-mcp or --mcp-url, not both");
      }
      await runBootstrap(
