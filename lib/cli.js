...
export async function runCli(argv = process.argv) {
  ...
  program
    .command("login")
    .description(
      "Store your API key in a local credentials file (~/.config/mrscraper on Unix). To get MrScraper API key, visit https://app.mrscraper.com/api-tokens. Prefer this over pasting secrets into agent prompts — agents can run `mrscraper login` and you type the key in the terminal.",
    )
  ...
  program
    .command("init")
    .description(
      "Interactive setup — same credential storage as login. To get MrScraper API key, visit https://app.mrscraper.com/api-tokens. Use when an agent needs a discoverable first-run command.",
    )
