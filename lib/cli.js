...
Auth: Run mrscraper login for browser OAuth, or set MRSCRAPER_API_KEY.
...
async function promptAndPersistApiKey() {
  console.log("Get a MrScraper API key at https://app.mrscraper.com/api-tokens");
  const response = await prompts({
    type: "password",
    name: "apiKey",
    message: "MrScraper API key (leave blank to skip)",
  });
...
export async function runCli(argv = process.argv) {
  ...
  program
    .command("login")
    .description("Sign in through browser OAuth, or store an API key explicitly.")
    .option("--api-key <key>", "Store this API key instead of using browser OAuth.")
    .option("--token <key>", "Deprecated alias for --api-key.")
    .option("--no-open", "Print the OAuth URL without opening a browser.")
    .action(async (options) => {
      ...
      await loginWithOAuth({ noOpen: options.open === false });
    });

  program
    .command("init")
    .description("Install the CLI, skills, and MCP integration for detected agents.")
    .option("--api-key <key>", "Store an API key non-interactively.")
    .option("--all", "Install skills and MCP for every detected agent harness (default).")
    .option(
      "--agent <name>",
      "Install skills and MCP for one agent harness.",
    ...
    .option("--skip-auth", "Skip the authentication check.")
    .option("--skip-skills", "Do not install the agent skill pack.")
    .option("--skip-mcp", "Do not register the MrScraper MCP server.")
  ...
  setup
    .command("mcp")
    .description("Install or refresh the MrScraper MCP integration.")
    .option("--all", "Install for every detected agent harness (default).")
    .option(
      "--agent <name>",
      "Install for one agent harness.",
    ...
    .option("--dry-run", "Print installation actions without changing the system.")
    .action((options) => {
      if (options.all && options.agent) {
        throw new Error("Use either --all or --agent, not both");
      }
  ...
  auth
    .command("status")
    .description("Show the active authentication method without exposing credentials.")
    ...
    .action((options) => {
      ...
      if (!status.authenticated) {
        console.log(`Not authenticated. Expected credentials at ${status.path}`);
      } else if (status.auth_type === "api_key") {
        console.log(
          status.source
            ? `Authenticated with an API key from ${status.source}.`
            : `Authenticated with an API key stored at ${status.path}`,
        );
      } else {
        const expiry = new Date(status.expires_at).toISOString();
        console.log(
          `Authenticated with OAuth (${status.expired ? "expired" : `expires ${expiry}`}) at ${status.path}`,
        );
      }
    });
  ...
  program
    .command("logout")
    .description("Revoke OAuth when possible and remove locally saved credentials.")
    .action(async () => {
      ...
      if (result.revocationError) {
        console.error(
          `Warning: local credentials were removed, but remote OAuth revocation failed: ${result.revocationError}`,
        );
      }
    });
  ...
  program
    ...
    .option("--json", "Always print machine-readable JSON.")
