...
async function runFetch(url, options) {
  ...
  if (options.superMode && !options.browserRendering) {
    throw new Error("--super-mode requires --browser-rendering");
  }
  const result = await runWithSpinner("Fetching page content…", () =>
    runWithAuth(options.token, (credential) =>
      fetchContentApi({
        ...
        superMode: Boolean(options.superMode),
...
function addFetchOptions(command) {
  return command
    ...
    .option(
      "--super-mode",
      "Send super=true for real-device browser rendering; requires --browser-rendering.",
      false,
    )
...
export async function runCli(argv = process.argv) {
  ...
  program
    .command("scrape")
    .description(
      "Create a saved AI scraper from one starting URL; use rerun for manual workflows or multiple targets.",
    )
    ...
    .option(
      "--mode <mode>",
      "AI execution mode: Cheap or Super; omit it to use the backend default.",
      choiceParser("mode", ["cheap", "super"]),
    )
    .option("--token <key>", "Override configured authentication with an API key.")
    .option(
      "--proxy-country <code>",
  ...
  program
    .command("rerun")
    .description(
      "Rerun a saved AI scraper or dashboard-built manual workflow on one or multiple URLs.",
    )
    .argument("<target>", "One URL, or comma/newline-separated URLs with --bulk.")
    .requiredOption(
      "--type <type>",
      "How the saved scraper was created: ai or manual.",
      choiceParser("type", ["ai", "manual"]),
    )
    .option(
      "--bulk",
      "Submit all parsed target URLs as one asynchronous bulk job.",
      false,
    )
    ...
    .option(
      "--max-depth <n>",
      "AI map crawl depth; omit it to use the saved scraper/backend default.",
      integerParser("max depth", 1),
    )
    .option(
      "--max-pages <n>",
      "AI maximum pages; omit it to use the saved scraper/backend default.",
      integerParser("max pages", 1),
    )
    .option(
      "--limit <n>",
      "AI maximum results; omit it to use the saved scraper/backend default.",
      integerParser("limit", 1),
    )
    .option("--include-patterns <regex>", "AI include URL regex.")
    .option("--exclude-patterns <regex>", "AI exclude URL regex.")
    .option("--proxy-country <code>", "AI proxy country code.")
    .option(
      "--max-retry <n>",
      "AI maximum retry count.",
      integerParser("max retry", 0),
    )
    .option(
      "--timeout <seconds>",
      "AI rerun timeout in seconds; used by listing reruns.",
      integerParser("timeout", 1),
    )
  ...
  program
    ...
    .option("--scraper-id <uuid>", "Exact saved scraper UUID filter.")
    .option(
      "--status <status>",
      "Exact result status: Draft, Finished, Running, Failed, or Cancelled.",
      choiceParser("status", ["draft", "finished", "running", "failed", "cancelled"]),
    )
    .option("--type <type>", "Exact result type filter, such as AI or Rerun-AI.")
    .option("--url <url>", "Exact stored target URL filter.")
          ...
          getAllResultsApi({
            ...
            status:
              options.status === undefined
                ? null
                : `${options.status[0].toUpperCase()}${options.status.slice(1)}`,
  ...
  program
    ...
    .option("--no-include-html", "Exclude stored HTML content from the response.")
    .option("--token <key>", "Override configured authentication with an API key.")
