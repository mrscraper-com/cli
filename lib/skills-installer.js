...
export const HARNESS_DEFINITIONS = Object.freeze([
  ...
  },
  {
    ...
    label: "Hermes Agent",
    detectPath: ".hermes",
    homeEnvironmentVariable: "HERMES_HOME",
    installerAgent: "hermes-agent",
  },
  {
    name: "opencode",
    label: "OpenCode",
    detectPaths: [".config/opencode", ".opencode"],
    xdgDetectPath: "opencode",
  },
  {
    name: "openclaw",
    label: "OpenClaw",
    detectPaths: [".openclaw", ".clawdbot", ".moltbot"],
  },
  {
    name: "pi",
    label: "Pi",
    detectPaths: [".pi/agent", ".pi"],
  },
  {
    name: "omp",
    label: "Oh My Pi",
    detectPath: ".omp",
    // The public `skills` CLI does not expose an `omp` target yet. Its Cline
    // target writes to ~/.agents/skills, which OMP supports as its canonical
    // standard-agent skills provider.
    installerAgent: "cline",
    installerNote:
      "Oh My Pi uses the standard ~/.agents/skills provider (the upstream installer currently exposes it through its `cline` target).",
  },
]);
...
/**
 * @param {(typeof HARNESS_DEFINITIONS)[number]} definition
 * @param {string} homeDirectory
 * @param {NodeJS.ProcessEnv} environment
 */
function harnessDetectionPaths(definition, homeDirectory, environment) {
  const relativePaths = definition.detectPaths || [definition.detectPath];
  const candidates = relativePaths.map((detectPath) =>
    path.join(homeDirectory, detectPath),
  );

  if (definition.homeEnvironmentVariable) {
    const configuredHome = environment[definition.homeEnvironmentVariable];
    if (configuredHome) candidates.unshift(configuredHome);
  }

  if (definition.xdgDetectPath) {
    const xdgConfigHome =
      environment.XDG_CONFIG_HOME || path.join(homeDirectory, ".config");
    candidates.unshift(path.join(xdgConfigHome, definition.xdgDetectPath));
  }
...
export function detectInstalledHarnesses(
  ...
  return HARNESS_DEFINITIONS.filter((definition) =>
    harnessDetectionPaths(definition, homeDirectory, environment).some(
      isDirectory,
    ),
  );
}
...
export function buildSkillsInstallArgs(agents, source = SKILL_SOURCE) {
  ...
  const installerAgents = [
    ...new Set(
      targetAgents.map((agent) => {
        const definition = HARNESS_DEFINITIONS.find(
          ({ name }) => name === agent,
        );
        return definition.installerAgent || definition.name;
      }),
    ),
  ];
  const args = [
  ...
  for (const agent of installerAgents) args.push("--agent", agent);
  args.push("--yes");
  return args;
}
...
/**
 * Install the MrScraper skill pack into one explicit harness or every detected
...
export function installMrscraperSkill(options = {}) {
  ...
  const installerNotes = [
    ...new Set(targets.map(({ installerNote }) => installerNote).filter(Boolean)),
  ];

  for (const note of installerNotes) log(`Note: ${note}`);

  if (dryRun) {
    log(`Would install the MrScraper skill pack for ${targetLabels}: ${rendered}`);
  } else {
    log(`Installing the MrScraper skill pack for ${targetLabels}…`);
