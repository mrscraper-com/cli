...
const PAGES = {
  ...
  noToken: htmlPage(
    ...
    "The callback did not include a code (or token) in the query string. Note: params after a # fragment never reach the CLI. Return to the terminal.",
  ),
  done: htmlPage(
...
/**
 * Run the loopback browser-login flow. Resolves with either a single-use
 * login code plus the PKCE verifier to exchange for a token, or (legacy)
 * the token itself when the web app sends one directly.
 * @param {{ appBaseUrl?: string, timeoutMs?: number, log?: (msg: string) => void }} [opts]
 * @returns {Promise<{ code?: string, codeVerifier?: string, token?: string }>}
 */
export function loginViaBrowser({
  ...
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      settle(() =>
        resolve(code ? { code, codeVerifier: pkce.verifier } : { token }),
      );
    });
