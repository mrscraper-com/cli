 ...
 * @returns {string}
 */
export function authLockPath(options = {}) {
  return path.join(configDir(options), "auth.lock");
}
...
function normalizeAuth(value) {
  ...
  if (authType === "oauth") {
    ...
    if (typeof value.token_type === "string" && value.token_type.trim()) {
      normalized.token_type = value.token_type.trim();
    }
