...
/** Legacy credentials location — still read as a fallback. @returns {string} */
...
export function authDir() {
  return path.join(os.homedir(), ".mrscraper");
}
...
export function loadSavedApiKey() {
  return readKeyFromFile(authPath()) ?? readKeyFromFile(credentialsPath());
}
...
export function clearSavedApiKey() {
  let removed = false;
  for (const file of [authPath(), credentialsPath()]) {
