...
/**
 * Run the complete bootstrap while keeping authentication owned by the CLI.
 *
 * @param {{
 *   agent?: string,
 *   apiKey?: string,
...
export async function runBootstrap(options = {}, dependencies = {}) {
  ...
  if (!options.skipAuth) {
    await attempt("Authentication", async () => {
      if (options.dryRun) {
        if (options.apiKey) {
          ...
          log("Would start MrScraper browser login.");
        }
        return;
      }
      if (options.apiKey) {
    ...
    log("Skipping authentication; run `mrscraper login` separately if needed.");
  }
