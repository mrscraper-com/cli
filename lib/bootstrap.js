...
import { resolveCliPackageSpec } from "./invocation-source.js";
...
export function installGlobalCli(options = {}) {
  const {
    ...
    packageSpec = resolveCliPackageSpec(version, environment),
...
export async function runBootstrap(options = {}, dependencies = {}) {
    ...
    await attempt("Authentication", async () => {
      if (options.dryRun) {
        if (options.apiKey) {
          ...
          log("Would leave authentication for `mrscraper login`.");
        } else {
          log("Would prompt for a MrScraper API key.");
        }
        return;
      }
      if (options.apiKey) {
      ...
      if (options.yes) {
        log(
          "Authentication not configured; run `mrscraper login` before web requests.",
        );
        return;
      }
      await authenticate();
  ...
  if (!options.skipSkills) {
    await attempt("Skill installation", () =>
