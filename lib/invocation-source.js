...
/**
 * Convert npm's GitHub package forms into a direct repository/branch skill
 * source understood by the public `skills` CLI.
 * @param {string} packageSpec
 */
export function skillSourceFromPackageSpec(packageSpec) {
  ...
  if (githubShorthand) {
    ...
    return ref
      ? `https://github.com/${owner}/${repository}/tree/${encodeGitHubRef(ref)}/skills/mrscraper`
      : `${owner}/${repository}`;
  }
